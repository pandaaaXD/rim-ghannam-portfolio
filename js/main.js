...
(function () {
  ...
  const I18N = {
    fr: {
      ...
      "mon.desc": "Construction d'un NOC restitué dans Grafana et affiché sur mur TV en mode kiosk, pour superviser un parc hébergé multi-clients. Trois chaînes de données indépendantes y convergent : les métriques infra PRTG (CPU / RAM / disque par serveur et par rôle), la disponibilité applicative synthétique mesurée par des scripts Bash qui poussent vers Loki, et les logs IIS / applicatifs remontés par la stack ELK (Filebeat → Logstash → Elasticsearch).",
      "mon.i1": "<strong>Mur TV NOC</strong> : dashboards Grafana 12 en mode kiosk, rotation par playlist — l'état du parc se lit en un coup d'œil",
    ...
    en: {
      ...
      "mon.desc": "Built a NOC rendered in Grafana and displayed on a TV wall in kiosk mode, monitoring a multi-tenant hosted fleet. Three independent data chains converge into it: PRTG infrastructure metrics (CPU / RAM / disk per server and role), synthetic application availability measured by Bash scripts pushing to Loki, and IIS / application logs shipped through the ELK stack (Filebeat → Logstash → Elasticsearch).",
      "mon.i1": "<strong>NOC TV wall</strong>: Grafana 12 dashboards in kiosk mode with playlist rotation — fleet health readable at a glance",
