...
(function () {
  ...
  const I18N = {
    fr: {
      ...
      "mon.role": "Conception &amp; maintenance du centre de supervision",
      "mon.desc": "Un NOC pensé en deux niveaux de lecture : un mur TV que l'équipe regarde de loin — vert, tout va bien ; rouge, on creuse — et des dashboards détaillés quand il faut investiguer. Grafana y réunit trois sources qui ne se parlent pas naturellement : les logs applicatifs (ELK), des tests de disponibilité maison (Bash → Loki) et les métriques serveurs (PRTG).",
      "mon.i1": "<strong>Le mur</strong> : la santé de toutes les infras et applications en temps réel, lisible en un coup d'œil depuis l'autre bout de la salle",
      "mon.i2": "<strong>Le dashboard client</strong> : on choisit un client et on voit le CPU, la RAM et le disque de chacun de ses serveurs",
      "mon.i3": "<strong>Sans rien casser</strong> : l'existant ne pouvait pas être modifié — tout l'enrichissement a été fait côté supervision",
      "mon.shot1": "// le mur TV — disponibilité et erreurs de toutes les infras : vert = OK, rouge = on creuse",
      "mon.shot2": "// le dashboard client — CPU / RAM / disque de chaque serveur, groupés par rôle",
    ...
    en: {
      ...
      "mon.role": "Designed &amp; maintain the operations center",
      "mon.desc": "A NOC designed around two levels of reading: a TV wall the team watches from across the room — green, all good; red, time to dig — and detailed dashboards for when investigation is needed. Grafana brings together three sources that don't naturally talk to each other: application logs (ELK), home-grown availability checks (Bash → Loki) and server metrics (PRTG).",
      "mon.i1": "<strong>The wall</strong>: the health of every infrastructure and application in real time, readable at a glance from across the room",
      "mon.i2": "<strong>The client dashboard</strong>: pick a client and see the CPU, RAM and disk of each of their servers",
      "mon.i3": "<strong>Without breaking anything</strong>: the existing systems couldn't be modified — all the enrichment was done on the monitoring side",
      "mon.shot1": "// the TV wall — availability and errors across every infrastructure: green = OK, red = dig in",
      "mon.shot2": "// the client dashboard — CPU / RAM / disk of each server, grouped by role",
